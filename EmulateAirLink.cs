...
namespace CuSensorArray
{
    ...
    public class EmulateAirLink
    {
        ...
        bool DoCalibrated = false;
        public double TemperatureC, TemperatureF, Humidity;
        ...
        public void DoAirLink()
        {
            ...
            if ( DoCalibrated ) { PM25_last = 14.8 + 0.3834 * SerialDevice.MinuteValues.Pm25_atm + -0.1498 * Humidity + -0.1905 * TemperatureC; }
            else PM25_last = SerialDevice.MinuteValues.Pm25_atm;

            // 14.7 + (0.3151 * PM10) + (-0.0948 * rHum) + (0.2445 * temperature)
            if ( DoCalibrated ) { PM10_last = 14.7 + 0.3151 * SerialDevice.MinuteValues.Pm10_atm + -0.0948 * Humidity + 0.2445 * TemperatureC; }
            else PM10_last = SerialDevice.MinuteValues.Pm10_atm;
